...
class UserEntrancePanel(wx.Panel):
    ...
    def update_password(self, event):
        ...
        if self.valid_password(self.password) and self.entrance_type is 'register':
            self.password_validation_text.SetLabel(' great password :) ')
    ...
    def server_verification(self):
        ...
        if server_response.startswith(SUCCESSFUL_ENTRY):  # if entry was successful:
            print server_response
            server_response = server_response.split(DELIMITER)
            CHAT_HISTORY = server_response[1]  # Previous chatting history.
            ALL_SONGS_LIST = server_response[2].split(DELIMITER_2)
...
class ChatPanel(wx.Panel):
    ...
    def __init__(self, parent_frame):
        ...
        self.choices = ['soon!', 'soon!', 'soon!']
        self.radio_box = wx.RadioBox(self, size=(200, 90), choices=self.choices,
                                     majorDimension=1, style=wx.RA_SPECIFY_COLS)

        self.radio_box.Bind(wx.EVT_RADIOBOX, self.on_radio_box)  # setting radio box.
    ...
    def on_radio_box(self, event):
        ...
        dlg = wx.MessageDialog(self, "Is " + self.radio_box.GetStringSelection() + " your final answer?",
                               "Confirm answer",
        ...
        if result == wx.ID_OK:  # if user chooses to leave
            if self.radio_box.GetStringSelection() == self.current_artist:  # if chosen answer is correct- add to score
                self.score += 1
            ...
            self.quiz_question.SetLabel("THE QUIZ IS ON! \n wait for the next song...")
    ...
    def receive_songs(self):
        ...
        while True:
            ...
            if self.parent_frame.stop_threads is True:  # if client has decided to leave party.
                ...
                song = song.split(DELIMITER)  # separates the song title from the rest of the string.
                ...
                if not self.parent_frame.started_playing:
                    self.parent_frame.started_playing = True
        ...
        while True:
            while SONGS_TO_PLAY:
                ...
                if str(time_passed) != '':
                    ...
                    self.mixer.play()  # playing song from the start.
            ...
            message = self.parent_frame.client.encrypt_request(
                (SEND_SCORE + DELIMITER + str(self.score)))
        ...
        while True:
            ...
            if self.parent_frame.stop_threads:  # if client has decided to disconnect:
                ...
                if message_received == USER_NOT_FOUND:
                    ...
                    self.print_score.SetLabel("WINNER: " + message_received.split(DELIMITER)[1] + "!")
                    self.quiz_question.SetLabel("QUIZ IS OVER!")
                    self.quiz_box.Layout()
...
class Frame(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.client = silent_disco_client.Client(60000)  # creating client.
        self.song_client = silent_disco_client.Client(60100)  # creating the song transfer client.
        ...
        self.started_playing = False  # true if started playing music, false otherwise.
        self.songs_to_play = []
